...
X = pd.get_dummies(X, columns=["weather", "year", "month", "hour"], drop_first=True)
...
grid_params = {
        ...
        "learning_rate": [0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 1.5, 2.0], 
...
grid_search = GridSearchCV(estimator=AdaBoostRegressor(), param_grid=grid_params, scoring=scorer, cv=5, n_jobs=-1, refit=True) 
