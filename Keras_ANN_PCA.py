...
def RMSLE(Y_test, Y_predict):
    return tf.math.sqrt(tf.math.reduce_mean(tf.math.square(tf.math.log1p(Y_test) - tf.math.log1p(Y_predict))))
...
X = pd.get_dummies(X, columns=["season", "weather", "month", "year", "hour"], drop_first=True)
...
final_score=model.evaluate(x=X_test, y=Y_test, batch_size=25)
print(final_score)
